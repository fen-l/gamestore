...
import { Heart, ShoppingCart, Star, Users, Clock, Award, Truck, Shield, X, Plus, Minus } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
          ...
          >
            <img src={game.images[imgIdx]} alt={game.title} className="w-full h-full object-cover" />
            {game.oldPrice && (
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            {game.isBestseller && <span className="px-2.5 py-1 text-xs font-bold rounded-full gradient-amber text-primary-foreground">ХИТ</span>}
            {game.isNew && <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-foreground text-background">NEW</span>}
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-1">
              {[1,2,3,4,5].map((s) => (
                <Star key={s} className={`w-5 h-5 ${s <= Math.round(game.rating) ? "fill-primary text-primary" : "text-muted"}`} />
              ))}
              <span className="ml-2 font-bold">{game.rating}</span>
            </div>
            <span className="text-sm text-muted-foreground">{game.reviewCount} отзывов</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {[
              { icon: Users, label: "Игроки", val: `${game.playersMin}–${game.playersMax}` },
              { icon: Clock, label: "Время", val: `${game.playTimeMin}–${game.playTimeMax} мин` },
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-6">
            <div className="flex items-end gap-3 mb-5">
              <div className="text-4xl font-extrabold">{game.price.toLocaleString("ru-RU")} ₽</div>
              {game.oldPrice && (
                <div className="text-lg text-muted-foreground line-through mb-1">{game.oldPrice.toLocaleString("ru-RU")} ₽</div>
              )}
              <div className="ml-auto text-sm">
                {game.inStock ? <span className="text-green-600 font-semibold">В наличии</span> : <span className="text-destructive">Нет в наличии</span>}
              </div>
            </div>

            <div className="flex gap-3">
              <div className="flex items-center bg-muted rounded-xl">
                <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="p-3 hover:bg-accent rounded-l-xl"><Minus className="w-4 h-4" /></button>
                <span className="px-4 font-bold w-10 text-center">{qty}</span>
                <button onClick={() => setQty((q) => q + 1)} className="p-3 hover:bg-accent rounded-r-xl"><Plus className="w-4 h-4" /></button>
              </div>
              <button
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="flex items-center gap-2 text-muted-foreground"><Truck className="w-4 h-4 text-primary" /> Доставка завтра</div>
            <div className="flex items-center gap-2 text-muted-foreground"><Shield className="w-4 h-4 text-primary" /> Гарантия качества</div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-16">
        <div className="flex gap-2 border-b border-border mb-6 overflow-x-auto">
          {([
            ["desc", "Описание"], ["specs", "Характеристики"], ["rules", "Правила"], ["reviews", `Отзывы (${game.reviews.length})`]
          ] as const).map(([k, l]) => (
            <button
              key={k}
        ...
        <div className="animate-fade-in">
          {tab === "desc" && <p className="text-foreground/80 leading-relaxed max-w-3xl">{game.description}</p>}
          {tab === "specs" && (
          ...
          {tab === "rules" && (
            <div className="prose max-w-3xl text-foreground/80 space-y-3">
              <p>Базовые правила игры включают подготовку поля, раздачу компонентов и по очереди ход каждого игрока.</p>
              <p>Полные правила прилагаются в коробке. Средняя длительность партии — {game.playTimeMin}–{game.playTimeMax} минут.</p>
            </div>
          )}
          {tab === "reviews" && (
            <div className="max-w-3xl space-y-6">
              ...
              <form
                onSubmit={(e) => { e.preventDefault(); setReviewText(""); alert("Спасибо за отзыв!"); }}
              ...
              >
                ...
                <div className="flex gap-1 mb-3">
                  {[1,2,3,4,5].map((s) => (
                    <button type="button" key={s} onClick={() => setReviewRating(s)}>
                      <Star className={`w-6 h-6 ${s <= reviewRating ? "fill-primary text-primary" : "text-muted"}`} />
                    </button>
                  ))}
                </div>
                <textarea
                ...
                />
                <button type="submit" className="mt-3 px-6 py-2.5 rounded-xl gradient-amber text-primary-foreground font-bold">
                  Отправить
                </button>
              </form>
            </div>
      ...
      {[
        ...
        <section key={sec.title} className="mt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {sec.items.map((g) => <GameCard key={g.id} game={g} />)}
          </div>
        </section>
      ))}
      ...
      {/* Lightbox */}
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4 animate-fade-in" onClick={() => setLightbox(false)}>
          <button className="absolute top-6 right-6 text-white p-2"><X className="w-6 h-6" /></button>
